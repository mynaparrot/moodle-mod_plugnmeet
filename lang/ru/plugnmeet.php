...
$string['video_codec_des'] = 'Стандарт кодирования для передачи видео. **VP9** — это стандарт по умолчанию и рекомендуемый для WebRTC, предлагающий превосходный баланс качества видео, эффективности процессора и низкой задержки (автоматически возвращаясь к **VP8** в старых браузерах). **Предупреждение:** Продвинутые кодеки, такие как AV1 или H.265, требуют поддержки современных браузеров; Убедитесь, что ваши пользователи подключены к совместимым настройкам, если вы отклоняетесь от стандартных.';
...
$string['recording_broadcasting_features'] = 'Запись и трансляция';
...
$string['allow_broadcasting'] = 'Разрешить вещание (Мастер-коммутатор)';
$string['allow_broadcasting_help'] = 'Основной мастер-коммутатор для всех функций вещания. Если отключено, все остальные опции трансляции будут отключены.';
...
$string['allow_reactions'] = 'Разрешённые реакции';
$string['allow_reactions_help'] = 'Позвольте участникам отправлять плавающие реакции (например, большой палец вверх, хлопки) во время сессии.';
...
$string['only_record_admin_webcams'] = 'Только веб-камеры администратора записей';
$string['only_record_admin_webcams_help'] = 'Если она включена, в запись будут включены только веб-камеры администраторов.';
...
$string['preassign_breakout_rooms'] = 'Заранее распределяйте комнаты по группам курсов';
$string['preassign_breakout_rooms_help'] = 'При включении каждая группа курса становится секционной комнатой с началом сессии: название комнаты — это название группы, и участники группы автоматически назначаются в неё. Модератор может в любой момент перестраивать или убирать эти задания в режиме управления секционной комнатой встречи. Пользователи, не входящие в группу, могут быть назначены модератором во время сессии. Это вступает в силу с начала следующей сессии, поскольку детали комнаты фиксируются при создании комнаты. Требуется, чтобы в курсе были группы.';
...
$string['enable_end_to_end_encryption_features_help'] = 'Обеспечивает надёжную безопасность за счёт шифрования видео- и аудиопотоков между участниками. В этом режиме сервер генерирует и безопасно распределяет уникальный ключ шифрования для сессии.';
$string['enabled_self_insert_encryption_key'] = 'Разрешить использование ключей, определяемых участниками.';
$string['enabled_self_insert_encryption_key_help'] = 'Позволяет участникам вручную вводить общий секретный ключ, обеспечивая высочайший уровень конфиденциальности, поскольку сервер никогда не имеет доступа к ключам шифрования. Ответственность ведущего — делиться одним и тем же секретом со всеми участниками; Если используются разные ключи, пользователи не смогут общаться друг с другом. <strong>Примечание:</strong> Когда эта опция включена, функции, требующие обработки медиа на сервере (такие как облачная запись, транскрипция и резюме с помощью ИИ), автоматически отключаются.';
...
$string['lock_reactions'] = 'Реакции замков';
$string['lock_reactions_help'] = 'Не позволяет участникам отправлять какие-либо реакции во время сессии.';
...
$string['client_files_missing'] = 'Клиентские файлы (JS/CSS) отсутствуют. Пожалуйста, проверьте конфигурацию сервера PlugNmeet.';
$string['client_connection_error'] = 'Произошла ошибка при подключении к серверу PlugNmet: {$a}. Пожалуйста, проверьте настройки плагинов.';
...
$string['analytics_room_shared_notepad_status'] = 'Счёт статуса блокнота';
...
$string['status_conflict'] = 'Это действие не могло быть выполнено из-за конфликта (например, повторение одного и того же запроса).';
...
$string['merged_file_name'] = 'Объединённое имя файла';
$string['merge'] = 'Слияние';
$string['close'] = 'Закрыть';
$string['merge_confirmation'] = 'Вы уверены, что хотите объединить выбранные записи?';
$string['modal_merge_recordings_header'] = 'Следующие записи будут объединены в таком порядке:';
$string['modal_merge_recordings_prompt'] = 'Для объединения необходимо выбрать как минимум две записи.';
$string['merge_request_success'] = 'Запрос на слияние записей успешно добавлен. Вы получите уведомление о завершении работы.';
$string['upload_to_whiteboard'] = 'Загрузка на доску';
$string['upload_to_whiteboard_success'] = 'Успешно загрузил выбранный файл на доску';
...
$string['poll_mode'] = 'Режим опроса';
$string['poll_mode_quiz'] = 'Викторина (показать правильный ответ после опроса)';
$string['poll_mode_vote'] = 'Голосуйте (собирайте только ответы)';
$string['poll_mode_help'] = 'В режиме викторины участники видят правильный ответ после окончания опроса. В режиме голосования правильный ответ не отображается.';
$string['poll_duration'] = 'Автозакрытие через (минуты)';
$string['poll_duration_help'] = 'Максимальный срок в 60 минут. 0 = без ограничения';
